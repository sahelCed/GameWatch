...
class MiniGameFactory {
    ...
    static func miniGameFrom(miniGameType: MiniGameType) -> MiniGame {
        ...
        case .search:
            ...
            return MiniGame(name: name!, instruction: instruction, isCompleted: false, type: .search)
        case .simonSays:
            let name = "Un étrange mécanisme à couleur"
            let instruction = "Répétez les séquences sur votre montre."
            return MiniGame(name: name, instruction: instruction, isCompleted: false, type: .simonSays)
        case .finalLock:
            let name = "Le tiroir secret du tyran"
            let instruction = "Entrez le code pour dévérouiller le tiroir."
            return MiniGame(name: name, instruction: instruction, isCompleted: false, type: .finalLock)
        case .exit:
            let name = "Porte de l'ESGI"
            let instruction = "Utilisez la clé pour sortir."
            return MiniGame(name: name, instruction: instruction, isCompleted: false, type: .exit)
        }
    }
}
